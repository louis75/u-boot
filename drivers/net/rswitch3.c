...
/*
 * Driver for Renesas Ethernet RSwitch3.
 *
 * Copyright (C) 2024 Renesas Electronics Corporation
 *
 * Based on the Renesas Ethernet RSwitch2 driver.
 */
...
#define RSWITCH_TX_CHAIN_INDEX		128
...
static void rswitch_agent_clock_ctrl(struct rswitch_priv *priv, int port, int enable)
{
	...
	if (enable) {
		...
		writel(val | RCEC_RCE | BIT(14), priv->addr + RCEC);
	} else {
		val = readl(priv->addr + RCDC);
		writel(val | BIT(14), priv->addr + RCDC);
	}
}
...
static __maybe_unused int rswitch_serdes_init(struct rswitch_etha *etha)
{
	int ret;
...
static int __maybe_unused rswitch_mii_access_c22_real(struct rswitch_etha *etha, bool read,
...
static int rswitch_mii_read(struct mii_dev *miidev, int phyad, int devad, int regad)
{
	...
	return rswitch_mii_access_c22(etha, true, phyad, regad, 0);
}
...
static int __maybe_unused rswitch_mii_read_c45(struct mii_dev *miidev, int phyad, int devad, int regad)
{
	struct rswitch_priv *priv = miidev->priv;
	struct rswitch_etha *etha = &priv->etha;
	int val;
...
static int rswitch_reset(struct rswitch_priv *priv)
{
	...
	if (!priv->parallel_mode) {
		...
		do {
			ret = rswitch_reg_wait(priv->addr, RSWITCH_GWCA0_OFFSET + GWMS,
...
{
	writel(GENMASK(RSWITCH_NUM_HW - 1, 0) | RCEC_RCE, priv->addr + RCEC);
}
...
static void rswitch_mfwd_init(struct rswitch_priv *priv)
{
	...
	writel(BIT(gwca->index), priv->addr + FWPBFC(etha->index));

	writel(BIT(etha->index), priv->addr + FWPBFC(gwca->index));
}
...
static int rswitch_gwca_init(struct rswitch_priv *priv)
{
	...
	writel(upper_32_bits((uintptr_t)priv->bat_desc), gwca->addr + GWDCBAC0);
	//writel(GWDCC_DQT | GWDCC_BALR, gwca->addr + GWDCC(RSWITCH_TX_CHAIN_INDEX));
	writel((0xff << 8) | 0xff, gwca->addr + GWMDNC);
	writel(GWTPC_PPPL(GWCA_IPV_NUM), gwca->addr + GWTPC0);
...
static int rswitch_send(struct udevice *dev, void *packet, int len)
{
	...
	writel(val | BIT(1), gwca->addr + GWTRC(gwtrc_index));
...
static int rswitch_probe(struct udevice *dev)
{
	...
	if (!s || port < 0 || port > RSWITCH_NUM_PORT - 1)
		port = CONFIG_RENESAS_ETHER_SWITCH3_DEFAULT_PORT;
	...
	gwca->index = 1;
	gwca->addr = priv->addr + RSWITCH_GWCA1_OFFSET;
